...
MAX_MICROUNITS = 40_000   # $0.04 cap
...
async def defi_analyze_endpoint(request: Request, body: DefiAnalyzeRequest):
    ...
    if is_api_key:
        if not credits.pre_deduct_max(request.state.human_api_key, MAX_MICROUNITS):
            analytics.track("defi", "api_key", False, 0, 0)
    ...
    result, tokens, _ = await cached_completion("defi", user_content, DEFI_SYSTEM_PROMPT, None, json_mode=True, max_tokens=get_max_tokens("defi"))
